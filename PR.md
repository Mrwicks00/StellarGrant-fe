...
- Added fully-typed `GrantData`, `GrantFund`, and `MilestoneData` interfaces whose field names match Soroban's `scValToNative` output (snake_case keys exactly as returned by the contract).
...
- Preserved `build:tsc` script as a fallback for projects that require the raw `.d.ts` emitter.
